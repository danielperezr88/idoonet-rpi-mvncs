...
class CocoPose(RNGDataFlow):
    ...
    def __init__(self, path, img_path=None, is_train=True, decode_img=True, only_idx=-1):
        ...
        if is_train:
            ...
            whole_path = os.path.join(path, 'banister_keypoints2_train.json')
        else:
            #whole_path = os.path.join(path, 'person_keypoints_val2017.json')
            whole_path = os.path.join(path, 'banister_keypoints2_val.json')
    ...
    def get_data(self):
        ...
        for idx in idxs:
            ...
            if 'http://' in self.img_path or self.img_path is None:
...
def get_dataflow(path, is_train, img_path=None):
    ...
    if is_train:
        ...
        ds = MultiThreadMapData(ds, nr_thread=16, map_func=read_image_url, buffer_size=10)#, buffer_size=1000)
