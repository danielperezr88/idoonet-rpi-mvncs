...
class TfPoseEstimator:
    ...
    def draw_humans(npimg, humans, imgcopy=False):
        ...
        image_h, image_w = npimg.shape[:2]
        centers = {}
        humans = humans if isinstance(humans, dict) else dict(zip(range(len(humans)),humans))
        for idx, human in humans.items():
            ...
            for pair_order, pair in enumerate(common.CocoPairsRender):
                ...
                # npimg = cv2.line(npimg, centers[pair[0]], centers[pair[1]], common.CocoColors[pair_order], 3)
                # cv2.line(npimg, centers[pair[0]], centers[pair[1]], common.CocoColors[pair_order], 3)
                cv2.line(npimg, centers[pair[0]], centers[pair[1]], common.CocoColors[idx], 3)

        return npimg

    @staticmethod
    def get_human_wrists(npimg, human):
    
        image_h, image_w = npimg.shape[:2]
        
        wrists = {4: None, 7: None}
        for idx, i in enumerate([4, 7]):
            if i not in human.body_parts.keys():
                continue
